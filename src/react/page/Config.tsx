...
const Config: FC = () => {
  ...
  const selecionarPasta = async () => {
    const result = await ninja.fileManager.openFolderSelection('Selecione a pasta de backup do postgres', form.values.dbBackupFolder || '');
    form.setFieldValue('dbBackupFolder', result);
  }
        ...
        <form onSubmit={form.onSubmit(formSubmit)}>
          ...
          <Tooltip label={form.values.dbBackupFolder || ''} disabled={!form.values.dbBackupFolder}>
            <Group mt="md" position="left">
                ...
                <ActionIcon variant="default" mt="xl" size="lg" onClick={selecionarPasta}>
